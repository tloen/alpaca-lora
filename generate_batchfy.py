...
def main(
    ...
    parser.add_argument("--lora_path",type=str,default="./alpaca")  # TODO: fix the `__init__() got an unexpected keyword argument 'enable_lora'` error when try to load the online lora weight from huggingface
    ...
    model.config.pad_token_id = tokenizer.pad_token_id # a newly added token
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        for i in tqdm(range(0, len(all_prompts), batch_size)):
            ...
            with torch.no_grad():
                # if the outputs are weird, try to use batch_size=1, cuz there are some pad tokens in the left of the input_ids when batchfy    
